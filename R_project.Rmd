...
knitr::kable(df_rmse_,col.names = c("Model","RMSE"), caption="Model performance with out-of-sample RMSEs (Bitcoin)")%>% 
    kable_classic_2(full_width=F)
```
...
knitr::kable(df_rmse__,col.names = c("Model","RMSE"), caption="Model performance with out-of-sample RMSEs (Ethereum)")%>% 
    kable_classic_2(full_width=F)
```
...
ggsave("price_compare.png", width = 6, height = 4, dpi = 300)
...
ggsave("vol_compare.png", width = 6, height = 4, dpi = 300)
