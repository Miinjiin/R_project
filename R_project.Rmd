...
knitr::include_graphics("price_compare.png")
```

```{r vol,message=FALSE, echo=FALSE, warning=FALSE}
# made from bitcoin_ether_volume below with ggplot
knitr::include_graphics("vol_compare.png")
```
...
# making sumtable with sumtable() and output=kable

sumstat_df=sumtable(df, out='return')

knitr::kable(sumstat_df)%>% kable_minimal() %>% kable_styling(full_width = FALSE, position="left", latex_options="scale_down")
...
```{r bitcoin_CART, message=FALSE, echo=FALSE, warning=FALSE, including=FALSE}
...
```{r ethereum_CART, message=FALSE, echo=FALSE, warning=FALSE, including=FALSE}
...
ggplot(compare, aes(x = DATE)) + 
  ...
  theme_economist_white(horizontal=FALSE, gray_bg=FALSE) + scale_colour_economist()

ggsave("price_compare.png", width = 10, height = 5, dpi = 300)
...
ggsave("vol_compare.png", width = 10, height = 5, dpi = 300)
...
# Appendix
```{r timeseries_tables, message=FALSE, echo=FALSE, warning=FALSE, including=FALSE}
