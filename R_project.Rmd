...
# making new column in a data frame that contains the difference between consecutive rows of an existing column
bitcoin_without_date$diff_column <- c(NA, diff(bitcoin_without_date$Bitcoin))
bitcoin_without_date$direction <- ifelse(bitcoin_without_date$diff_column > 0, 1, 0)
bitcoin_without_date <- na.omit(bitcoin_without_date)

head(bitcoin_without_date)
...
bitcoin_RF=randomForest(Bitcoin ~ . -direction, data=bitcoin_train,  important=TRUE)

varImpPlot(bitcoin_RF)

bitcoin_direction_RF=randomForest(direction ~ .-diff_column-Bitcoin, data=bitcoin_train,  important=TRUE)

varImpPlot(bitcoin_direction_RF)
```
...
```{r  bitcoin_RF.2, message=FALSE, echo=FALSE, warning=FALSE, include=FALSE}

phat=predict(bitcoin_direction_RF, bitcoin_without_date, type = "response")

accuracy= foreach(i=1:90, .combine='c') %do% {
  yhat_val_best=ifelse(phat_val_best >= (i/100), 1, 0)
  confusion_val_best=table(y=bitcoin_without_date$direction, yhat=yhat_val_best)
...
i=1:90
yhat=ifelse(phat_val_best >= (i/100), 1, 0)
confusion = table(y=bitcoin_without_date$direction, yhat=yhat)
view(confusion)
TN=confusion[1,1]
FP=confusion[1,2]
FN=confusion[2,1]
...
roc_plot = foreach(t = t, .combine='rbind')%do%{
  yhat = ifelse(phat >= t, 1, 0)
  confusion_best_val = table(y=bitcoin_without_date$direction, yhat=yhat)
